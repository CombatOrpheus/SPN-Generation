...
function _initialize_petri_net_opt(num_places::Int, num_transitions::Int)
    ...
    # Instead of fully shuffling the array which allocates memory or does O(N) ops
    # We can do an in-place Fisher-Yates partial shuffle later, but we need the array shuffled completely here
    # Actually shuffle! doesn't allocate much, it does in-place swapping
    shuffle!(remaining_nodes)

    sub_places = Vector{Int}(undef, num_places)
    sub_transitions = Vector{Int}(undef, num_transitions)
    sub_places[1] = first_place
    sub_transitions[1] = first_transition
