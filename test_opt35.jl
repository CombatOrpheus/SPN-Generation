...
function get_enabled_transitions_opt!(pre_condition_matrix, change_matrix, current_marking_vector, enabled_transitions_buffer, new_markings_buffer, place_upper_limit)
    ...
    num_enabled = 0
    # Optimization: Transpose pre_condition_matrix iteration?
    # Usually pre_condition_matrix is small (10x10).
    # What if we invert the loop so we iterate over places first?
    # No, we need to check per transition.
