...
function write_to_hdf5(group, data; compression="gzip", compression_opts=4)
    for (key, value) in data
        try
            ...
            @warn "Could not save key '$key' for sample $(HDF5.name(group)). Error: $e"
        end
    end
end
...
function run_generation_from_config(config)
    ...
    if isempty(all_samples)
        @warn "No samples were generated. Skipping file writing and reporting."
        return
    end
    ...
    if output_format == "hdf5"
        h5open(output_path, "w") do hf
            ...
            @info "Writing $(length(all_samples)) samples to HDF5..."
