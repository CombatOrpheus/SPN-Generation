...
function _generate_candidate_matrices_core_opt(
    ...
    # Calculate exact number of matrices to pre-allocate
    num_delete_edge = 0
    num_add_edge = 0
...
pn[:, end] = rand(0:3, 10)
println("Original:")
b = @benchmarkable _generate_candidate_matrices_core(copy(pn), true, true, true, true) setup=(pn=$pn)
display(run(b))
println("Optimized:")
b2 = @benchmarkable _generate_candidate_matrices_core_opt(copy(pn), true, true, true, true) setup=(pn=$pn)
display(run(b2))
