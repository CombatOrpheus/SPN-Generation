...
## 2026-05-15 - Contiguous Matrix Updates Avoid Cache Misses in BFS Loops
**Learning:** In hot loops where matrices are repeatedly updated (like in ), writing to a matrix with standard column-major inner loop iteration over columns is cache aligned for the write, but reading an input matrix via the varying column index forces non-contiguous reads, causing many cache misses. Furthermore, hashing a sliced row view later is slightly slower than hashing a sliced column view.
**Action:** Transpose or reshape the buffer matrix (e.g. from  to ). Loop with the row index on the inside to make ALL matrix accesses completely contiguous across both read () and write () arrays. Slicing column views for downstream hashing is also faster.
